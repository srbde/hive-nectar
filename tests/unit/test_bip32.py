...
def test_btc_privkey():
    ...
    for idx in (44 + BIP32_HARDEN, 0 + BIP32_HARDEN, 0 + BIP32_HARDEN, 0, 0):
        bip32_child_key_obj = _require_child(bip32_child_key_obj.ChildKey(idx))
    assert bip32_child_key_obj.Address() == "1A9vZ4oPLb29szfRWVFe1VoEe7a2qEMjvJ"
    assert (
...
def test_with_sec():
    ...
    for idx in (44 + BIP32_HARDEN, 0 + BIP32_HARDEN, 0 + BIP32_HARDEN, 0 + BIP32_HARDEN, 0):
        m = _require_child(m.ChildKey(idx))
    assert (
        m.ExtendedKey()
        == "xprvA3Fu8ZNFZDn3S24jWzHCLGsX9eSUcpvFY2FFKLzessSEkk1KQLhHpyG7rnfYtx7txBupUY546PT5tjb4kwXghpd1rRw1Xw8nAqS19EZuPSu"
    )
...
def test_parse_path():
    ...
    assert bin_path == b"800000308000000d800000008000000080000000"
